...
static func build(
	...
	var frame := _build_frame(frame_w, frame_h, resolved_frame_surface)
	door_root.add_child(frame)

	# Build door panel on hinge
	if connection.type == "double_door":
		_build_double_panel(door_root, frame_w, frame_h, resolved_panel_surface)
	else:
		_build_single_panel(door_root, frame_w, frame_h, resolved_panel_surface)
...
static func _build_frame(width: float, height: float, surface_ref: String = "") -> Node3D:
...
static func _build_single_panel(parent: Node3D, width: float, height: float, surface_ref: String = "") -> void:
	...
	var panel := _make_door_panel(width, height, surface_ref)
...
static func _build_double_panel(parent: Node3D, total_width: float, height: float, surface_ref: String = "") -> void:
	...
	var panel_l := _make_door_panel(panel_width, height, surface_ref)
	...
	var panel_r := _make_door_panel(panel_width, height, surface_ref)
...
static func _make_door_panel(width: float, height: float, surface_ref: String = "") -> Node3D:
