...
- blas_create(geometries: RDAccelerationStructureGeometry[]) -> RID - Creates a new Bottom Level Acceleration Structure. It can be accessed with the RID that is returned. Once finished with your RID, you will want to free the RID using the RenderingDevice's `free_rid` method.
...
**AccelerationStructureGeometryFlagBits:** ACCELERATION_STRUCTURE_GEOMETRY_OPAQUE_BIT=1, ACCELERATION_STRUCTURE_GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION_BIT=2
  - ACCELERATION_STRUCTURE_GEOMETRY_OPAQUE_BIT: An opaque geometry does not invoke the any hit shaders.
  - ACCELERATION_STRUCTURE_GEOMETRY_NO_DUPLICATE_ANY_HIT_INVOCATION_BIT: This geometry only calls the any hit shader a single time for each primitive.
